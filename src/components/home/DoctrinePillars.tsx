...
export default function DoctrinePillars() {
    ...
    return (
        <>
            {/* Desktop Layout (Standard Hover behavior) */}
            <div
                className="hidden lg:flex w-full h-[600px] flex-row relative bg-[#0D2C51] overflow-hidden rounded-md"
                ...
                {PILLARS.map((pillar) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            />
                            <div className="relative z-20 w-full h-full flex flex-col items-center justify-center p-12 overflow-hidden">
                                <AnimatePresence>
                                    {!isActive && (
                                        <motion.div
                                            key="vertical-label"
                                        ...
                                        >
                                            <h3
                                                className="text-7xl font-bold tracking-[0.15em] uppercase select-none whitespace-nowrap"
                                                style={{
                                ...
                                {isActive && (
                                    <div className="flex flex-col items-start justify-center h-full w-full max-w-xl">
                                        ...
                                        >
                                            ...
                                            <h3 className="text-7xl font-bold text-white mb-8 tracking-wide">
                                                {pillar.title}
                                            </h3>
            ...
            {/* Mobile Layout (Strict Single-Column, Non-Destructive) */}
            <div className="flex lg:hidden flex-col gap-4 w-full">
                {PILLARS.map((pillar) => (
                    <div
                        key={pillar.id}
                        className="bg-[#0D2C51] p-8 rounded-md border-l-4 border-white/20"
                    >
                        <span className="block text-white/40 text-[10px] font-bold tracking-[0.2em] uppercase mb-4">
                            SECTION 005 : {pillar.title}
                        </span>
                        <h3 className="text-4xl font-bold text-white mb-6 tracking-wide">
                            {pillar.title}
                        </h3>
                        <p className="text-base text-white/90 leading-relaxed font-light mb-8">
                            {pillar.body}
                        </p>
                        <div className="border-l-2 border-white/20 pl-4">
                            <p className="text-white font-medium italic text-sm tracking-wide">
                                "{pillar.sub}"
                            </p>
                        </div>
                    </div>
